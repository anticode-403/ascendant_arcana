...
public class EmiInfusionRecipe implements EmiRecipe {
    ...
    public EmiInfusionRecipe(InfusionRecipe recipe, ItemStack relic) {
        ...
        this.recipe = recipe;
        this.id = recipe.getId().withPrefixedPath("/").withSuffixedPath("/" + RelicItem.getRelicType(relic).name().toLowerCase());
        this.template = EmiStack.of(AArcanaItems.INFUSION_SMITHING_TEMPLATE);
        this.input = EmiIngredient.of(Ingredient.ofStacks(Registries.ITEM.stream().filter((item) -> recipe.testBase(new ItemStack(item)) && recipe.matches(new ItemStack(item), relic)).map(ItemStack::new)));
